...
- [Contacts](#contacts)
- [Orders](#orders)
- [Coupon Templates](#coupon-templates)
- [Coupons](#coupons)
- [Info](#info)
...
Feed's root element (**products**, **orders**, etc.) can contain an optional attribute **totalItems** containing the total number of filtered items of the current entity.
It means all the available entities after filtering (applied *since* date) before applying pagination (*limit* and *offset*).

Wrong value of **totalItems** can break paging algorithm and cause feed to be unreadable. It's better to not include the field at all rather than providing wrong value.

### Version

Version of the feed schema definition you've implemented. Optional.

Current version of feed schema: **4.0**

### Generator

Name and version of the program or plugin that generated the feed. Optional.

### XML Header Example

```
<?xml version="1.0" encoding="UTF-8" ?>
<products version="4.0" totalItems="12859" generator="Magento2:5.1.18">
...
```
...
| Attribute Name | Required | Type              | Description                  |
|----------------|----------|-------------------|------------------------------|
| **key**        | optional | [string](#string) | Category or type of the tag. |
...
| **customFields**     | optional     | [GroupedCustomFields](#groupedcustomfields) | Possible custom fields.                              |
...
>
    <c id="1">
        ...
        <customFields>
	        <f key=”CategoryImage” group=”trait”>https://www.myshop.cz/img/123.png</f>
	        <f key=”CategoryImportance” group=”trait”>100</f>
	        <f key=”Temporary”>1</f>
        </customFields>
    </c>
    <c id="2">
...
Validate with [XML Schema Definition](xsd/categories.xsd).
...
| Attribute Name | Required | Type              | Description                           |
|----------------|----------|-------------------|---------------------------------------|
| **country**    | optional | [string](#string) | An ISO country code                   |
| **currency**   | optional | [string](#string) | An ISO currency code.                 |
...
| **measurePrice**       | optional     | [decimal](#decimal) | Unit measure price. Decimal fractions are supported; use a dot as the XML decimal separator.                                                                               |
| **measureAmount**      | optional     | [decimal](#decimal) | Unit measure amount, e.g. 1, 10, 100, or 1.5. Decimal fractions are supported; use a dot as the XML decimal separator.                                                    |
| **measureUnitName**    | optional     | [string](#string)   | Unit measure name, e.g. kg, g, or barel.                                                                                                                                  |

#### Example

```xml
<prices>
    <p country="CZ" currency="USD" pricelist="cenik1">
        <amount>129</amount>
        <priceWithoutTax>111</priceWithoutTax>
        <priceAfterDiscount>99</priceAfterDiscount>
        <discountRate>15</discountRate>
        <measurePrice>150</measurePrice>
        <measureAmount>1</measureAmount>
        <measureUnitName>kg</measureUnitName>
    </p>
</prices>
```
...
Root element tag: **imageUrls**

Each image URL is wrapped in an **i** child element.

##### Example

```xml
<imageUrls>
    <i>https://images.books.com/space-program-front.jpg</i>
    <i>https://images.books.com/space-program-back.jpg</i>
</imageUrls>
```
...
| Element Name        | Required     | Type                                    | Description                                                                                                                                   |
|---------------------|--------------|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| **imageUrls**       | optional     | [ProductImages](#productimages)         | URLs of images of the product.                                                                                                                |
...
| **purchase**        | optional     | [decimal](#decimal)                     | Purchase price of the stock. In an optional attribute **currency**, you can set the currency of the price.                                    |
...
| **sku**             | optional     | [string](#string)                       | Stock Keeping Unit                                                                                                                            |
...
| **newFlag**         | optional     | [integer](#integer)                     | *Deprecated — use [tags](#tags) instead. Still supported.* The product is flagged new (**0** - no, **1** - yes).                              |
| **specialDealFlag** | optional     | [integer](#integer)                     | *Deprecated — use [tags](#tags) instead. Still supported.* The product is flagged a special deal (**0** - no, **1** - yes).                   |
| **choiceFlag**      | optional     | [integer](#integer)                     | *Deprecated — use [tags](#tags) instead. Still supported.* The product is flagged a choice - recommended product (**0** - no, **1** - yes).   |
...
| **validFrom**       | optional     | [datetime](#datetime)                   | Product valid from date                                                                                                              |
| **validTo**         | optional     | [datetime](#datetime)                   | Product valid to date                                                                                                                |
...
>
    <p id="P257">
        ...
        <sku>123ABC</sku>
        <imageUrls>
            <i>https://images.books.com/space-program.jpg</i>
        </imageUrls>
        <categories>
            <c>1</c>
            <c>2</c>
...
Validate with [XML Schema Definition](xsd/products.xsd).
...
| **companyName**  | optional | [string](#string)                           |                                                                   |
...
<contacts
    version="4.0"
    totalItems="6040"
    generator="NasaSpaceFeeds:1.2.3"
>
    <c id="e28bc2f0">
...
| **price**           | **required** | [decimal](#decimal) | Unit price of the item. Tax inclusive. Optional attribute **currency** overrides the order currency. |
| **priceWithoutTax** | optional     | [decimal](#decimal) | Unit price of the item without tax.    |
...
| **contact**         | optional     | [Contact](#contact)       | Contact details in case of unregistered sale. (Use only when **contactId** is missing) |
...
| **shipping**        | optional     | [string](#string)         | The means of shipping.                                                                 |
...
| **customFields**    | optional     | [GroupedCustomFields](#groupedcustomfields) | A map of custom fields where any order-related value can be stored.   |
...
>
    <o id="88886">
        ...
        <payment>with Moon coins</payment>
        <shipping>by rocket</shipping>
        <total>284</total>
        <totalWithoutTax>234</totalWithoutTax>
        <currency>USD</currency>
        <items>
            <i id="111836">
                <quantity>1</quantity>
                <price currency="EUR">199</price>
                <priceWithoutTax>164.46</priceWithoutTax>
            </i>
        </items>
        <customFields>
            <f key="Store" group="logistics">External</f>
            <f key="Delivery">DHL</f>
        </customFields>
    </o>
    ...
</orders>
...
Validate with [XML Schema Definition](xsd/orders.xsd).

### Coupon Templates

Each coupon must be assigned to a single template. This feed provides a list of all coupon templates present in the e-shop.

If your platform doesn't support coupon templates, instead of implementing this feed, you should use [ad-hoc coupons](#creating-new-coupons).

Root element tag: **couponTemplates**

#### Coupon Template

A single coupon template.

All attributes except **id** are marked as optional, but you must provide them either in the CouponTemplate feed or provide all necessary attributes on per-coupon basis in Coupon feed (**discountType**, **reusable**, **discount**, **validFrom**, **validTo**).

Root element tag: **couponTemplate**

| Element Name     | Required     | Type                  | Description                                                                                                                                                                                                                        |
|------------------|--------------|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **id**           | **required** | [string](#string)     | Unique identifier of coupon template.                                                                                                                                                                                              |
| **title**        | optional     | [string](#string)     | Optional human-readable name of the coupon template.                                                                                                                                                                               |
| **reusable**     | optional     | [integer](#integer)   | Whether this type of coupon can be used multiple times. **0** - no, **1** - yes. Reusable coupons are typically used as a single code for multiple users. Non-reusable coupon is usually personalized, and we can track its usage. |
| **discountType** | optional     | [string](#string)     | Possible values: **PERCENTUAL**, **MONETARY**, **FREE_SHIPPING**                                                                                                                                                                   |
| **discount**     | optional     | [decimal](#decimal)   | Either the **MONETARY** or **PERCENTUAL** value of the coupon.                                                                                                                                                                             |
| **minOrder**     | optional     | [decimal](#decimal)   | **FREE_SHIPPING**  minimal order amount.                                                                                                                                                                                                |
...
>
    ...
    <couponTemplate>
        <id>156</id>
        <title>Christmas Campaign 2024</title>
        <reusable>0</reusable>
        <discountType>MONETARY</discountType>
    ...
    <couponTemplate>
        ...
        <reusable>0</reusable>
        <discountType>FREE_SHIPPING</discountType>
        <minOrder>2000</minOrder>
    </couponTemplate>
    ...
</couponTemplates>
```

Validate with [XML Schema Definition](xsd/couponTemplates.xsd).

### Coupons

A list of discount coupons (aka *vouchers*).

If your platform doesn't support coupon templates, instead of implementing this feed, you should use [ad-hoc coupons](#creating-new-coupons).

Root element tag: **coupons**

#### Coupon

A single coupon.

All attributes except **code** are marked as optional, but you must provide either **couponTemplateId** that references a template from [Coupon Templates](#coupon-templates) or provide all necessary attributes on per-coupon basis (**discountType**, **reusable**, **discount**, **validFrom**, **validTo**).

Normally, coupons are generated on the e-shop side and sent to Incomaker. Incomaker distributes the coupons to the users by e-mail, SMS, etc. E-shop does not allocate the coupons to a particular user until the user use it. We call such coupons "unassigned". Alternatively, sometimes could be useful to have the coupons allocated to the particular user while generating it. We call such coupons "assigned".

Root element tag: **coupon**

| Element Name         | Required      | Type                  | Description                                                                                                                                                                                                                                                           |
|----------------------|---------------|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **couponTemplateId** | optional      | [string](#string)     | Unique identifier of the coupon type. Must match **id** in [Coupon Templates](#coupon-templates) feed. Required when not providing **discountType**, **reusable**, **discount**, **validFrom**, and **validTo** directly.                                              |
| **code**             | **required**  | [string](#string)     | The actual coupon code that a customer will use when claiming the coupon.                                                                                                                                                                                             |
| **reusable**         | optional      | [integer](#integer)   | Whether this type of coupon can be used multiple times. **0** - no, **1** - yes. Reusable coupons are typically used as a single code for multiple users. Non-reusable coupon is usually personalized, and we can track its usage.                                    |
| **discountType**     | optional      | [string](#string)     | Possible values: **PERCENTUAL**, **MONETARY**, **FREE_SHIPPING**                                                                                                                                                                                                     |
| **discount**         | optional      | [decimal](#decimal)   | Either the **MONETARY** or **PERCENTUAL** value of the coupon.                                                                                                                                                                                                        |
| **minOrder**         | optional      | [decimal](#decimal)   | **FREE_SHIPPING**  minimal order amount.                                                                                                                                                                                                                              |
| **contactId**        | optional      | [string](#string)     | Coupon recipient. Optional. An Id of the contact in the e-shop that owns the coupon, must match **id** attribute in the contact feed. This is used, for example, when coupon is for customer's birthday and hence is reserved for this customer since it was created. |
...
>
    <coupon>
        <couponTemplateId>16</couponTemplateId>
        <code>GHCJKD</code>
    </coupon>
    <coupon>
        <discountType>PERCENTUAL</discountType>
        <code>D45s45</code>
        <discount>20</discount>
        <contactId>1789</contactId>
        <validFrom>2020-04-10T00:00:00</validFrom>
        <validTo>2025-04-17T00:00:00</validTo>
    </coupon>
    ...
</coupons>
```

Validate with [XML Schema Definition](xsd/coupons.xsd).

### Creating New Coupons

If you want Incomaker to send coupons within an email campaign, we can use this method to create coupons on the fly. 
If you implement this, there is no need to also implement [Coupon Templates](#coupon-templates) and [Coupons](#coupons). 

To create coupons, Incomaker will make a **POST** request to your standard base URL with following parameters:

| Param Name   | Required     | Type                          | Description                                                                                                                                      |
|--------------|--------------|-------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|
| **key**      | **required** | [string](#string)             | API key returned from the register plugin function. API key has a format of [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier)  |
| **type**     | **required** | [string](#string)             | Indicates type of returned feed. The only supported value here is: **coupon**.                                                                   |
| REQUEST_BODY | **required** | [RequestBody](#RequestBody)   | JSON document with attributes for new coupons.                                                                                                   |

This endpoint returns XML document with identical schema to [Coupons](#coupons) feed containing all generated coupons. However, there is no paging or filtering support in this case.

#### RequestBody

Request body contains [JSON](https://www.json.org/json-en.html) document that specifies attributes for new coupons - either provide ID of coupon template or all necessary attributes.

You must either provide template ID matching **couponTemplateId** that references a template from [Coupon Templates](#coupon-templates) or provide all necessary attributes on per-coupon basis (**discountType**, **reusable**, **discount**, **validFrom**, **validTo**).

| Attribute Name       | Required     | Type                  | Description                                                                                                                                                                                                                                                                                                                               |
|----------------------|--------------|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **amount**           | required     | [integer](#integer)   | If there are assigned coupons in the request, this attribute cannot be smaller than their amount, otherwise it is omitted. If this amount is higher than the number of assigned coupons, the rest of the coupons up to the this amount will be generated as unassigned coupons.                                                                                                                                                                    |
| **couponTemplateId** | optional     | [string](#string)     | An identifier of coupon type as in [Coupon Templates](#coupon-templates) feed.                                                                                                                                                                                                                                                            |
| **discountType**     | optional     | [string](#string)     | Possible values: **PERCENTUAL**, **MONETARY**, **FREE_SHIPPING**                                                                                                                                                                                                                                                                          |
| **reusable**         | optional     | boolean               | Whether this type of coupon can be used multiple times. Reusable coupons are typically used as a single code for multiple users. Non-reusable coupon is usually personalized, and we can track its usage.                                                                                                                                 |
| **discount**         | optional     | [decimal](#decimal)   | Either the **MONETARY** or **PERCENTUAL** value of the coupon.                                                                                                                                                                                                                                                                                    |
| **minOrder**         | optional     | [decimal](#decimal)   | **FREE_SHIPPING** minimal order amount.                                                                                                                                                                                                                               |
| **validFrom**        | optional     | [datetime](#datetime) |                                                                                                                                                                                                                                                                                                                                           |
| **validTo**          | optional     | [datetime](#datetime) |                                                                                                                                                                                                                                                                                                                                           |
| **contactIds**       | optional     | Array&lt;string&gt;   | Coupon recipients. Optional. If specified, then there should be a coupon created for each contact. Array of contact IDs in the e-shop that will own created coupons, must match **id** attribute in the contact feed. This is used, for example, when creating birthday coupons. Only applicable to coupon templates that are "assigned". |

#### Example - by template ID

Following example will generate 1000 new coupons from template **DiscountChristmas2024**:

    POST https://www.myeshop.com/feed.php?key=92342b04-fa97-4fde-baa5-c3f912b88430&type=coupon
    {
        "amount": 1000,
        "couponTemplateId": "DiscountChristmas2024"
    }

#### Example - by attributes

Following example will generate 500 new coupons for 10% discount:
    ...
    POST https://www.myeshop.com/feed.php?key=92342b04-fa97-4fde-baa5-c3f912b88430&type=coupon
    {
        ...
        "validTo": "2024-01-30T00:00:00"
    }

#### Example - for specific contacts

Following example will generate new coupon for discount of 150 (in e-shop's currency) for each of the contacts (**86342b04** and **4fde001**):
    ...
    POST https://www.myeshop.com/feed.php?key=92342b04-fa97-4fde-baa5-c3f912b88430&type=coupon
    {
        ...
        "validFrom": "2023-12-01T00:00:00",
        "validTo": "2024-01-30T00:00:00",
        "contactIds": ["86342b04", "4fde001"]
    }

### Info

> **Deprecated.** This feed is no longer the recommended way to provide additional e-shop information.
...
Validate with [XML Schema Definition](xsd/info.xsd).

## XML Feed Validation

It is a time-saver for both you and us if you verify your feeds before submitting them to Incomaker.

There are XML Schema Definition files inside `xsd` folder of this repository that will help you validate your feeds.
...
If you have Docker installed, you can use enclosed container and scripts to validate XML feed:
